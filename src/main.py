...
async def index():
    ...
    html_path = os.path.join(workspace_path, "assets", "lesson_plan_web.html")
    
    if os.path.exists(html_path):
        return FileResponse(html_path, media_type="text/html")
    else:
        return HTMLResponse(content="<h1>教案生成助手</h1><p>HTML文件未找到</p>")
...
async def test_page():
    """测试页面"""
    workspace_path = os.getenv("COZE_WORKSPACE_PATH", "/workspace/projects")
    html_path = os.path.join(workspace_path, "tests", "test_web.html")
    
    if os.path.exists(html_path):
        return FileResponse(html_path, media_type="text/html")
    else:
        return HTMLResponse(content="<h1>测试页面</h1><p>测试文件未找到</p>")
